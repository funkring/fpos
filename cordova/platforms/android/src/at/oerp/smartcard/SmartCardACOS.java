...
public class SmartCardACOS extends AbstractCashRegisterSmartCard {
	...
	public SmartCardACOS(Card card) throws IOException {
		super(card);
	...
	public byte[] doSignatur(byte[] sha256Hash, String pin) throws IOException {
		...
		CommandAPDU command = card.createAPDU(0x00, 0x22, 0x41, 0xb6, TLV);
		executeCommand(command);
		return doSignaturWithoutSelection(sha256Hash, pin);
	}

	public byte[] doSignaturWithoutSelection(byte[] sha256Hash, String pinS) throws IOException {
		...
		command = card.createAPDU(data);
		executeCommand(command);
		command = card.createAPDU(0x00, 0x2A, 0x90, 0x81, sha256Hash);
		executeCommand(command);
		command = card.createAPDU(0x00, 0x2A, 0x9E, 0x9A, 256);
	...
	public X509CertificateHolder getCertificate() throws IOException {
		...
		if ( dataList == null || dataList.isEmpty() ) throw new SmartCardException("Buffer is empty");
	...
	private long getCertificateSerial() throws IOException {
