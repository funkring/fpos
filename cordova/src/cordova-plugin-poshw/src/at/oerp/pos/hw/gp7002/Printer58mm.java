...
public class Printer58mm extends PosHwPrinter implements CtrlBytes, LinePrintDriver  {
	...
	// service
	private GP7002PosService service;
	// printer api
	 ...
	 * @throws IOException
	 */
	public Printer58mm(GP7002PosService inService) throws SecurityException, IOException {
		service = inService;
		charset = Charset.forName("ASCII");
		driver = new Cprinter();
		empty = "".getBytes(charset);
	}
	...
	public void writeln(String inText) throws IOException {
		if ( inText != null ) {
			driver.printString(inText.getBytes(charset), inText.length());
		} else {
			driver.printString(empty, 0);
		}
	}
